...
def plot(spectra_contents, data_contents, distance, delay, x_type, y_type, plot_scale,
         spectra_names, spectra_last_modified_time, data_names, data_last_modified_time,
         output_style):
    print(spectra_contents)
    error_div_list = []
    ...
    if data_contents is not None:
        df_data, error_div_list = parse_content(content=data_contents, name=data_names, error_div_list=error_div_list)
        ...
        # Plot
        try:
            ax1 = df_spectra.set_index(keys='X').plot(legend=False, ax=ax1)
        except TypeError:
            pass
        ...
        return None, html.Div([dcc.Graph(figure=plotly_fig, id=app_id_dict['plot_fig_id'])]), output_style
    else:
        return None, plot_loading, output_style
