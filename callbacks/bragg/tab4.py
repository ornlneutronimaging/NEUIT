...
@app.callback(
    [
        ...
        Output(app_id_dict['hidden_texture_add_row_time_tab4'], 'children'),
        Output(app_id_dict['texture_table_tab4'], 'data'),
        Output(app_id_dict['texture_checklist_tab4'], 'value'),
        Output(app_id_dict['grain_size_input_tab4'], 'value'),
        Output(app_id_dict['grain_size_checklist_tab4'], 'value'),
    ],
    [
        Input(app_id_dict['cif_upload_tab4'], 'filename'),
        Input(app_id_dict['add_row_tab4'], 'n_clicks_timestamp'),
        Input(app_id_dict['texture_add_row_tab4'], 'n_clicks_timestamp'),
    ],
    [
        ...
        State(app_id_dict['hidden_texture_add_row_time_tab4'], 'children'),
        State(app_id_dict['texture_checklist_tab4'], 'value'),
        State(app_id_dict['texture_table_tab4'], 'data'),
        State(app_id_dict['texture_table_tab4'], 'columns'),
...
def upload_feedback(cif_names, add_button_timestamp, texture_add_button_timestamp,
    ...
    if file_uploads is not None:
        ...
        for _row_index, _row in enumerate(_cif_struc):
            ...
            _new_table_entry = {constants.atom: atom,
...
# texture checklist
@app.callback([
                Output(app_id_dict['texture_add_row_tab4'], "disabled"),
                Output(app_id_dict['texture_table_tab4'], "editable"),
                ],
              Input(app_id_dict['texture_checklist_tab4'], "value"))
def method(checklist_tab4_value):

    # button is unchecked
    if len(checklist_tab4_value) == 1:
        return True, False
...
@app.callback(
    Output(app_id_dict['grain_size_input_tab4'], "disabled"),
    Input(app_id_dict['grain_size_checklist_tab4'], "value"))
def method_grain_size_tab4(checklist_value):
